...
# my %data;
my $inFN = $cgi -> param('input');
my $inFH = $cgi -> upload('input');
my $adj = $cgi -> param('adj');
my $scale = $cgi -> param('scale');
my $seqflag = $cgi -> param('seq');
# my $
# $data{}
...
# my $adj=3;
# my $f_rates;
my $f_fasta = "../ref/human_g1k_v37.fasta";
# my $f_positions;
# my $f_rates;
my $seqflag;
# my $outfile;
...
my $subseq = $adj*2+1;
my $f_rates = "../ref/ERV_${subseq}bp_rates.txt";
...
while(<$inFH>){
  ...
  # print "$categ\t$catind\n";
	...
	if($localseq!~/[MNSW]/){
      ...
      print "$linestr\t$sequence\t$hash{$sequence}[$catind]\t\n";
		}	else {
      # print "$linestr\n";
      # my $rate=$hash{$sequence}[$catind];
      # print "$linestr\t$sequence\t$rate\n";
      # print OUT "$linestr\t$hash{$sequence}[$catind]\n";
      print "$linestr\t$hash{$sequence}[$catind]\n";
		}
	}
